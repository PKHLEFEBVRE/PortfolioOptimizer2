...
Sub ReportWorkflow()
    ...
    Set wsDash = ThisWorkbook.Sheets("Positions Dashboard")
    On Error GoTo 0

    If wsDash Is Nothing Then
        MsgBox "Positions Dashboard not found. Run the optimizer first.", vbExclamation
        Exit Sub
    End If
    ...
    ' Find the tables dynamically
    Dim r As Long
    Dim posRow As Long, weightRow As Long
    posRow = 0
    weightRow = 0

    For r = 1 To lastRow
        If wsDash.Cells(r, 1).Value = "Asset Name" Then posRow = r
        If wsDash.Cells(r, 1).Value = "Portfolio Strategy" Then
            ' The second "Portfolio Strategy" is the Weights table
            If weightRow = 0 Then
                ' This is the Metrics table, skip
                weightRow = -1 ' Just a dummy flag
            Else
                ' If we find it a second time, it's the Weights table
                weightRow = r
            End If
        End If
    Next r

    ' We actually know exactly how to find them.
    ' Asset Name is posRow.
    ' Weight table is the very last block of data.
    Dim wStart As Long
    ...
    If posRow = 0 Or wStart = 0 Then
        MsgBox "Could not find data tables on Positions Dashboard.", vbExclamation
        Exit Sub
    End If

    ' Extract Asset Names and basic metrics from Positions Table
    Dim assetsData As Object
    Set assetsData = CreateObject("Scripting.Dictionary")

    Dim aName As String
    r = posRow + 1
    Do While wsDash.Cells(r, 1).Value <> "" And wsDash.Cells(r, 1).Value <> "SIMULATED PORTFOLIOS"
        aName = CStr(wsDash.Cells(r, 1).Value)
        If aName <> "" Then
            Dim innerDict As Object
            Set innerDict = CreateObject("Scripting.Dictionary")
            innerDict.Add "Asset Name", aName
            innerDict.Add "Current Price", wsDash.Cells(r, 2).Value
            innerDict.Add "Conviction (/5)", wsDash.Cells(r, 13).Value
            innerDict.Add "Target Price", wsDash.Cells(r, 14).Value
            innerDict.Add "Stop Price", wsDash.Cells(r, 16).Value
            innerDict.Add "Expected Return", wsDash.Cells(r, 6).Value
            assetsData.Add aName, innerDict
        End If
        r = r + 1
    Loop

    ' Extract Weights from Weights Table
    Dim c As Long, lastCol As Long
    lastCol = wsDash.Cells(wStart, wsDash.Columns.Count).End(xlToLeft).Column

    Dim stratName As String
    For r = wStart + 1 To lastRow
        stratName = CStr(wsDash.Cells(r, 1).Value)
        If stratName <> "" Then
            For c = 2 To lastCol
                aName = CStr(wsDash.Cells(wStart, c).Value)
                ' The header is "W_Asset" or just the asset name now
                If Left(aName, 2) = "W_" Then aName = Mid(aName, 3)

                If assetsData.Exists(aName) Then
                    assetsData(aName).Add stratName, wsDash.Cells(r, c).Value
                End If
            Next c
        End If
    Next r
