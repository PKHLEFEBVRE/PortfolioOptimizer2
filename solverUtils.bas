...
Sub WriteToEngine(covMat() As Double, expReturns() As Double, maxHHI As Double)
    ...
    Call SetupOptimizationFormulas(wsEng, nAssets, solverRow, maxHHI)
End Sub

' ---------------------------------------------------------
' 1. SETUP: Added Log Calculation Rows for the "Log Trick"
' ---------------------------------------------------------
Sub SetupOptimizationFormulas(wsEng As Worksheet, nTotal As Long, startRow As Long, maxHHI As Double)
    ...
    Dim rf As Double
    ' Attempt to get RF natively to remove Dash dependency
    ...
    End If
    On Error GoTo 0
    If rf = 0 Then rf = 0.02 ' Default to 2% if missing
    ...
    If maxHHI <= 0 Then maxHHI = 4 ' default
    wsEng.Cells(fRow + 7, 2).Value = 1 / maxHHI
