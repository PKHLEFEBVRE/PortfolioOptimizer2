...
Sub RunAllSolvers()
    InitializeGlobals
    Application.ScreenUpdating = False
    Sheets(config.EngineSheet).Activate
    ...
    Set wsEngC = Sheets(config.EngineSheet)
    ...
    For i = LBound(strategies) To UBound(strategies)
        ...
        Dim w As Variant
        w = Sheets(config.EngineSheet).Range("OptWeights").Value
    ...
    Sheets(config.EngineSheet).Visible = False
    Sheets(config.DashSheet).Activate
...
Sub RunClean()
    ...
    Sheets(config.ChartSheet).Cells.ClearContents
    Sheets(config.WeightsSheet).Cells.ClearContents
End Sub